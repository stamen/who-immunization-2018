...
var PolioDataProvider =
/*#__PURE__*/
function () {
  function PolioDataProvider(props) {
    _classCallCheck(this, PolioDataProvider);

    this.rows = props.polioChartData.map(function (d) {
      return {
        total: +d["Denominator"],
      ...
      return (0, _d.descending)(a.protectedBothPercentage, b.protectedBothPercentage);
    });
    this.countries = this.rows.reduce(function (countries, d) {
      countries.push(d.country);
      return countries;
    }, []);
    this.hierarchy = (0, _d.hierarchy)({
      children: this.rows.map(function (d) {
        return {
          children: [{
            name: "protectedBoth",
  ...
  _createClass(PolioDataProvider, [{
    key: "getRows",
  ...
  return PolioDataProvider;
}();

var _default = PolioDataProvider;
exports.default = _default;
},{"d3":"../node_modules/d3/index.js"}],"sketches/polio_bars/index.js":[function(require,module,exports) {
"use strict";

var d3 = _interopRequireWildcard(require("d3"));

var _polio_chart = _interopRequireDefault(require("/data/polio_chart.csv"));

var _PolioDataProvider = _interopRequireDefault(require("/components/PolioDataProvider"));
...
document.addEventListener("DOMContentLoaded", function () {
  Promise.all([d3.csv(_polio_chart.default)]).then(function (_ref) {
    var _ref2 = _slicedToArray(_ref, 1),
        polioChartData = _ref2[0];

    var polioDataProvider = new _PolioDataProvider.default({
      polioChartData: polioChartData
    });
    var rows = polioDataProvider.getRows();
    var margin = {
      top: 80,
      right: 80,
      bottom: 80,
    ...
    var padding = 0.1,
        outerPadding = 0.3,
        x = d3.scaleBand().padding(0).domain(polioDataProvider.getCountries()).range([0, width]);
    var totalExtents = d3.extent(rows, function (d) {
      return d.total;
    });
    var y = d3.scaleLinear().range([height, 0]).domain(totalExtents);
    var barWidthScale = d3.scaleLinear().domain(totalExtents).range([0, x.bandwidth()]);
    var xAxis = d3.axisBottom().tickFormat(function (d) {
      return labelTextTransform[d] || d;
    }).scale(x);
    var yAxis = d3.axisLeft().scale(y);
    var svg = d3.select("body").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"); // Draws x axis

    svg.append("g").attr("transform", "translate(0, ".concat(height, ")")).call(xAxis); // Draws y axis

    svg.append("g").call(yAxis);

    var barWidth = function barWidth(d) {
      return barWidthScale(d.total);
    };

    var barx = function barx(d) {
      return x(d.country) + 0.5 * x.bandwidth() - 0.5 * barWidth(d);
    }; // Draws bars


    var outerBars = svg.selectAll(".bar").data(rows).enter().append("rect").attr("class", "bar").attr("x", barx).attr("width", barWidth).attr("y", function (d) {
      return y(d.total);
    }).attr("height", function (d) {
      return height - y(d.total);
    });
    outerBars.append("rect").attr("x", barx).attr("width", barWidth).attr("y", function (d) {
      return y(d.protectedBoth);
    }).attr("height", function (d) {
      return height - y(d.protectedBoth);
    });
  });
});
},{"d3":"../node_modules/d3/index.js","/data/polio_chart.csv":"data/polio_chart.csv","/components/PolioDataProvider":"components/PolioDataProvider.js"}],"../../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js","sketches/polio_bars/index.js"], null)
//# sourceMappingURL=/polio_bars.862a850e.js.map