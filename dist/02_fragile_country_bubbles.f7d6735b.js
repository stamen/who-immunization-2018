...
var FragileBubbleDataProvider =
/*#__PURE__*/
function () {
  function FragileBubbleDataProvider(_ref) {
    var fragileCountryBubbleData = _ref.fragileCountryBubbleData;

    _classCallCheck(this, FragileBubbleDataProvider);

    this.rows = fragileCountryBubbleData.map(function (d) {
      return {
        country: d.Country,
        status: d.Status,
      ...
      return (0, _d.ascending)(a.unvaccinated, b.unvaccinated);
    });
  }

  _createClass(FragileBubbleDataProvider, [{
    key: "getRows",
  ...
  return FragileBubbleDataProvider;
}();

var _default = FragileBubbleDataProvider;
exports.default = _default;
},{"d3":"../node_modules/d3/index.js"}],"components/renderHelpers.js":[function(require,module,exports) {
"use strict";
...
},{}],"02_fragile_country_bubbles/index.js":[function(require,module,exports) {
"use strict";

var d3 = _interopRequireWildcard(require("d3"));

var _fragile_country_bubbles = _interopRequireDefault(require("/data/fragile_country_bubbles.csv"));

var _FragileBubbleDataProvider = _interopRequireDefault(require("./FragileBubbleDataProvider"));
...
document.addEventListener("DOMContentLoaded", function () {
  Promise.all([d3.csv(_fragile_country_bubbles.default)]).then(function (_ref) {
    var _ref2 = _slicedToArray(_ref, 1),
        fragileCountryBubbleData = _ref2[0];

    var fragileBubbleDataProvider = new _FragileBubbleDataProvider.default({
      fragileCountryBubbleData: fragileCountryBubbleData
    });
    var data = fragileBubbleDataProvider.getRows();
    var height = 1280,
        width = 2048,
        margin = {
      top: 100,
    ...
    var legendContainer = plotSVG.append("g").attr("transform", "translate(25,25)");

    var x = function x(d) {
      return d.dtp3;
    };

    var xscale = d3.scaleLinear().domain([0, 100]).range([0, chartWidth]);
    var statuses = ["Fragile", "Polio", "Others"];
    var statusColors = ["red", "orange", "turquoise"];
    var statusChannelHeight = chartHeight / statuses.length;
    var colorScale = d3.scaleOrdinal().domain(statuses).range(statusColors);

    var status = function status(d) {
      return d.status;
    };

    var statusChannelScale = d3.scaleOrdinal().domain(statuses).range(d3.range(statuses.length));

    var unvaccinated = function unvaccinated(d) {
      return d.unvaccinated;
    };

    var radiusScale = d3.scaleSqrt().domain(d3.extent(data, unvaccinated)).range([0, 120]);

    var isLabeled = function isLabeled(d) {
      return ["Fragile", "Polio"].includes(status(d)) || unvaccinated(d) > 400000 || x(d) < 60;
    };
    ...
    var axisEl = container.append("g").attr("transform", "translate(0,0)").attr("class", "axis x").call(xAxis).style("font-size", "24px");
    [axisEl.selectAll("g.tick line"), axisEl.selectAll("path.domain")].forEach(function (s) {
      return s.attr("opacity", "0.2");
    });
    var nodes = data.map(function (d) {
      var r = radiusScale(unvaccinated(d));
      var x0 = xscale(x(d));
      var y0 = statusChannelScale(status(d)) * statusChannelHeight + 0.5 * statusChannelHeight + (Math.random() - 0.5) * statusChannelHeight;
    ...
    container.selectAll("g.bubble").data(nodes, function (d) {
      return d.country;
    }).join(function (enter) {
      var g = enter.append("g").attr("class", "bubble").attr("transform", function (d) {
        return "translate(".concat(d.x, ",").concat(d.y, ")");
      });
      g.append("circle") //.attr("cx", d => d.x0)
      //.attr("cy", d => d.y0)
      .attr("r", function (d) {
        return d.r;
      }).attr("fill", function (d) {
        return colorScale(status(d));
      });
      g.append("text").text(function (d) {
        return isLabeled(d) ? d.country : "";
      });
    });
    (0, _renderHelpers.addSVGDownloadLinkBeneath)(plotSVG);
  });
});
},{"d3":"../node_modules/d3/index.js","/data/fragile_country_bubbles.csv":"data/fragile_country_bubbles.csv","./FragileBubbleDataProvider":"02_fragile_country_bubbles/FragileBubbleDataProvider.js","/components/renderHelpers":"components/renderHelpers.js"}],"../../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js","02_fragile_country_bubbles/index.js"], null)
//# sourceMappingURL=/02_fragile_country_bubbles.f7d6735b.js.map