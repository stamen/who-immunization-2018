...
var HIGH_INCOME_STATUSES = ["high income: nonoecd", "high income: oecd"];

var GaviDataProvider =
/*#__PURE__*/
function () {
  function GaviDataProvider(props) {
    _classCallCheck(this, GaviDataProvider);

    this.gaviData = props.gaviData.map(function (d) {
      var income_status = d.income_status;
  ...
  return GaviDataProvider;
}();

var _default = GaviDataProvider;
exports.default = _default;
},{}],"components/renderHelpers.js":[function(require,module,exports) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.addSVGDownloadLinkBeneath = exports.renderImageBeneath = void 0;

var renderImageBeneath = function renderImageBeneath(svgSelection, height, width) {
  ...
  var context = canvas.getContext("2d"); // Convert SVG to Canvas
  // see: https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Drawing_DOM_objects_into_a_canvas

  var DOMURL = window.URL || window.webkitURL || window; // Get the string representation of a DOM node (removes the node)

  var domNode = svgSelection.node(); // Inserts canvas after the node

  domNode.insertAdjacentElement('afterend', canvas);
  var svgString = new XMLSerializer().serializeToString(domNode);
  var image = new Image();
...
exports.renderImageBeneath = renderImageBeneath;

var addSVGDownloadLinkBeneath = function addSVGDownloadLinkBeneath(svgSelection) {
  var domNode = svgSelection.node();
  var svgAsXML = new XMLSerializer().serializeToString(domNode);
  var dataURL = "data:image/svg+xml,".concat(encodeURIComponent(svgAsXML));
...
exports.addSVGDownloadLinkBeneath = addSVGDownloadLinkBeneath;
...
},{"_css_loader":"../../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/css-loader.js"}],"sketches/gavi_groups/index.scss":[function(require,module,exports) {
var reloadCSS = require('_css_loader');

module.hot.dispose(reloadCSS);
module.hot.accept(reloadCSS);
},{"_css_loader":"../../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/css-loader.js"}],"sketches/gavi_groups/index.js":[function(require,module,exports) {
"use strict";

var d3 = _interopRequireWildcard(require("d3"));

var _gavi_explicit_rows = _interopRequireDefault(require("/data/gavi_explicit_rows.csv"));

var _GaviDataProvider = _interopRequireDefault(require("./GaviDataProvider"));

var _renderHelpers = require("/components/renderHelpers");
...
var ROOT_COLOR = "#FFFFFF"; // Defines the GAVI status possible values.  NOTE: The order here determines
// order in the circle and is used in the color scales below.

var uniqueGaviStatusList = ["NON GAVI", "Transitioned ", "Transitioning", "GAVI"]; //const uniqueIncomeStatusList = [
//"Low Income",
//"lower middle income",
//"upper middle income",
//"high income"
...
var hue = 204;
var gaviStatusColorScale = d3.scaleOrdinal(uniqueGaviStatusList, ["hsl(".concat(hue, ", 25%, 80%)"), "hsl(".concat(hue, ", 100%, 72%)"), "hsl(".concat(hue, ", 100%, 72%)"), "hsl(".concat(hue, ", 75%, 60%)") //`hsl(${hue}, 85%, 68%)`,
]);
var gaviStatusBorderColor = d3.scaleOrdinal(uniqueGaviStatusList, ["hsl(".concat(hue, ", 25%, 90%)"), "hsl(".concat(hue, ", 100%, 82%)"), "hsl(".concat(hue, ", 100%, 82%)"), "hsl(".concat(hue, ", 75%, 70%)") //`hsl(${hue}, 85%, 78%)`,
]);

var getLabelText = function getLabelText(d) {
  var label = d.data.key || d.data.country_name;
...
function drawGAVISunburst(root) {
  ...
  var plotSVG = d3.select("body").append("svg").attr("class", "gavi_sunburst").attr("width", width).attr("height", height);
  var container = plotSVG.append("g").attr("transform", "translate(".concat(0.5 * width, ", ").concat(0.5 * height, ")"));

  var getInnerRadius = function getInnerRadius(d) {
    return d.y0;
  };

  var getOuterRadius = function getOuterRadius(d) {
    return d.y1;
  };
  ...
  var arc = d3.arc().startAngle(function (d) {
    ...
    return d.x1;
  }).innerRadius(getInnerRadius).outerRadius(getOuterRadius);

  var isRadialLabel = function isRadialLabel(d) {
    return d.x1 - d.x0 > 0.035;
  };

  var labelTransform = function labelTransform(d) {
    var theta = d.x0 + (d.x1 - d.x0) / 2;

    if (isRadialLabel(d)) {
      var r = getInnerRadius(d) + (getOuterRadius(d) - getInnerRadius(d)) / 2;
      var rotation = (theta - Math.PI / 2) / Math.PI * 180;
      return "rotate(".concat(rotation, ") translate(").concat(r, ",0) rotate(").concat(theta < Math.PI ? 0 : 180, ")");
    } else {
      var _r = getOuterRadius(d) + 12;

      return "translate(".concat(_r * Math.cos(theta - Math.PI / 2), ",").concat(_r * Math.sin(theta - Math.PI / 2), ")");
    }
  };

  var labelClass = function labelClass(d) {
    if (isRadialLabel(d)) {
  ...
  var labelAnchor = function labelAnchor(d) {
    var theta = d.x0 + (d.x1 - d.x0) / 2;
  ...
  var nodes = partition(root).descendants(); // filters nodes to keep only those large enough to see.
  ...
  var getGaviStatusForNode = function getGaviStatusForNode(d) {
    switch (d.depth) {
      case 1:
        return d.data.key;
  ...
  container.selectAll("path.sunburst_path").data(nodes).enter().append("svg:path").attr("class", "sunburst_path").attr("display", function (d) {
  ...
  var labelNodes = nodes.slice(1); //const labelNodes = [nodes[1]];

  container.selectAll("text.sunburst_label").data(labelNodes).enter().append("text").attr("class", labelClass).attr("transform", labelTransform) //.attr("width", d => getOuterRadius(d) - getInnerRadius(d))
  .attr("dy", function (d) {
    return isRadialLabel(d) ? 0.25 : 2;
  }).attr("text-anchor", labelAnchor).attr("opacity", function (d) {
    return isRadialLabel(d) ? 1.0 : 0.0;
  }).text(getLabelText).call(wrap, 72);
  var innerCircleContainer = container.append("g").attr("class", "sunburst_inner_circle");
  var rootNode = nodes[0];
  innerCircleContainer.data([rootNode]).append("circle").attr("r", function (d) {
    return d.y1;
  });
  var centerTitleContainer = d3.select("body").data([rootNode]).append("div").attr("class", "title_container").style("left", function (d) {
    return 0.5 * width - d.y1 + 6;
  }).style("top", function (d) {
    return 0.5 * height - 0.5 * d.y1 + 6;
  }).style("width", function (d) {
    return 2 * d.y1;
  });
  centerTitleContainer.append("h1").text("19.4 Million Unprotected Children");
  (0, _renderHelpers.addSVGDownloadLinkBeneath)(plotSVG);
}

document.addEventListener("DOMContentLoaded", function () {
  Promise.all([d3.csv(_gavi_explicit_rows.default)]).then(function (_ref) {
    var _ref2 = _slicedToArray(_ref, 1),
        gaviData = _ref2[0];

    var gaviDataProvider = new _GaviDataProvider.default({
      gaviData: gaviData
    ...
    var gaviNodes = d3.nest().key(function (d) {
      return d.gavi_status;
    }).sortKeys(function (a, b) {
      return uniqueGaviStatusList.indexOf(a) < uniqueGaviStatusList.indexOf(b);
    }).key(function (d) {
      return d.income_status;
    }).entries(gaviDataProvider.getRows()); // Turn the data into a d3 hierarchy and calculate the sums.
...
},{"d3":"../node_modules/d3/index.js","/data/gavi_explicit_rows.csv":"data/gavi_explicit_rows.csv","./GaviDataProvider":"sketches/gavi_groups/GaviDataProvider.js","/components/renderHelpers":"components/renderHelpers.js","/index.scss":"index.scss","./index.scss":"sketches/gavi_groups/index.scss"}],"../../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../../../../../../../usr/local/lib/node_modules/parcel/src/builtins/hmr-runtime.js","sketches/gavi_groups/index.js"], null)
//# sourceMappingURL=/gavi_groups.fd1abd93.js.map